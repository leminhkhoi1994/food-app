import { NA, NEGATIVE_TYPE, NOT_VALID_NUMBER, NUMBER_NOT_AVAILABLE } from "../../../constants/number"
...
export const cleanNumberArr = (numbers = []) => {
  const arrUpdated = numbers.filter((elem) => !NOT_VALID_NUMBER.includes(elem))
...
export const numberWithCommas = (number, roundedDigit) => {
  ...
  updatedNumber =
    splittedNumber[0].replace(/.(?=(?:.{3})+$)/g, "$&,") + (splittedNumber.length === 2 ? `.${splittedNumber[1]}` : "")

  return isNegativeNumber(number) ? `-${updatedNumber}` : updatedNumber
}

export const addBracketsForNumber = ({ number, roundedDigit, negativeType }) => {
  if (NOT_VALID_NUMBER.includes(number)) {
    return NUMBER_NOT_AVAILABLE
  }
...
export const isValidNumber2 = (number) => {
  const numberRegex = /^(-?)(\d{1,9})?(\.|$)(\d{1,4})?$/
