 ...
 */
const manifest = (self.__WB_MANIFEST.filter(({ url }) => !ignored.includes(url)) || []).concat([
  {
 ...
 */
registerRoute(
  ({ url }) => [""].includes(url.origin),
  new CacheFirst({
    cacheName: "api-response",
    plugins: [new CacheableResponsePlugin({ statuses: [0, 200, 201] })],
  }),
)
...
registerRoute(
  ({ url, request }) =>
    !ignored.includes(url.pathname) && (request.destination === "script" || request.destination === "style"),
