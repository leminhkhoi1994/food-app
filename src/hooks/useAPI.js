...
import { getData, postData, putData, patchData, deleteData } from "services/api"
...
const useAPI = () => {
  ...
  const onCallAPI = useCallback(
    async ({ isFile = false, method, params, data: dataBody, url, options = {}, callback }) => {
      setData((prev) => ({
        ...prev,
      ...
      try {
        ...
        const { message = [] } = error
        const errorMessage = error?.response?.data?.Data?.[0]?.Message || message || "Something went wrong!"
